...
impl FtpResponse {
	/// Parses the IP and port information from a `PASV` response.
	///
	/// The `PASV` response contains a section of data enclosed inside of parentheses.
	/// This data is in the following format: (h1,h2,h3,h4,p1,p2).
	/// The data becomes the ip address and port through `h1.h2.h3.h4:((p1 * 256) + 256)`.
	///
	/// ## Example:
	/// ```rust
...
///
/// Once calling the constructor a connection to the specified socket address
/// is opened, allowing for login and additional operations to be performed.
///
/// ## Example:
/// Connection
/// ```rust
...
impl FtpConnection {
	...
	///
	/// After initiating the stream of data to the server, the
	/// function waits for a response telling it if the service is available
	/// or if the service is not yet ready. If and once the server is ready,
	/// the function will return a functional instance of FtpConnection ready for
	/// varius operations to be performed.
	///
	/// ## Example:
	...
	/// Authenticates the connection to the FTP server.
	///
	/// When connecting to an FTP server the first command expected
	/// is the `USER` command specifying a username. From there a
	/// password should then be given logging the user in if their
	/// credentials were correct. A failure to authenticate will return
	/// an error of the type `FtpError::NotLoggedIn`.
	///
	/// ## Example:
	/// Login to 127.0.0.1 then close the connection.
	/// ```rust
	/// use ftp_lib::FtpConnection;
	/// use std::net::SocketAddrV4;
	///
	...
	///
	/// As long as no connection errors occur while sending the quit
	/// command or shutting down both read and write interfaces,
	/// the connection will be severed, despite what the servers response
	/// is.
	///
	/// ## Example:
	...
	///
	/// Works similarly to the `cd` command in shells. On successful
	/// completion of this command, `pwd` should often follow to be
	/// absolutely sure the program knows where it currently is, this
	/// is due to the fact that the client cannot be certain of how the
	/// server will behave in response.
	///
	/// ## Example:
	...
	///
	/// Simmilar to the cd function, but simply goes up to the parent directory.
	/// This function is the same as `cd ..` in most shells.
	///
	/// ## Example:
	...
	///
	/// The working directory is important to keep track of in order
	/// To orient the client in the server's file system. The working directory
	/// is the default place most commands are executed from unless instructed
	/// otherwise.
	///
	/// ## Example:
	...
	///
	/// Listing the files in the directory can be a difficult task,
	/// if the server being used does not follow the common output,
	/// the program may not be able to understand the current directory.
	/// If there is an issue, another directory list structure may need to
	/// be implemented.
	///
	/// ## Example:
	/// Lists all files in /
	...
	///
	/// This function has a definite output unlike `LIST`.
	/// However, less detailed information is returned.
	/// Only the names of the files are returned from this function.
	///
	/// ## Example:
	/// Lists all files in /
	/// ```rust
